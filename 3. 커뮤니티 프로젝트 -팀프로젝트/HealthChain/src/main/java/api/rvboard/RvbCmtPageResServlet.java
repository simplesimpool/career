...
@WebServlet("/RvbCmtPageResServlet")
public class RvbCmtPageResServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public RvbCmtPageResServlet() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        PagingData cmtPgData = rvbCmtDao.getPagingData(cmtPage, rvbId);
        
        if (cmtPage > cmtPgData.getMaxPage()) {
        	cmtPgData = rvbCmtDao.getPagingData(cmtPgData.getMaxPage(), rvbId);
        } else if (cmtPage < 1) {
        	cmtPgData = rvbCmtDao.getPagingData(1, rvbId);
        }
