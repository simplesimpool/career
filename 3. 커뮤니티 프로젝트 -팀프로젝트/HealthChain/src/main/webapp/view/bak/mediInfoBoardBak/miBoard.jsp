...
<body>
	...
	<table border="1">
		...
		<c:forEach var="board" items="${boardList}" varStatus="status">
			<tr>
				<td colspan="5">
					<div class="board-container" onclick="location.href='miBoardContent.sg?page=${pgData.page}&id=${board.getMib_id()}'">
	...
	<div class="page-container">
		<c:if test="${pgData.startPage != 1 }">
			<button onclick="window.location='miBoard.sg?page=${pgData.prevPage }'">이전</button>
		</c:if>
		<c:forEach var="i" begin="${pgData.startPage }" end="${pgData.endPage }">
			<button onclick="window.location='miBoard.sg?page=${i }'">${i }</button>
		</c:forEach>
		<c:if test="${pgData.endPage != pgData.maxPage }">
			<button onclick="window.location='miBoard.sg?page=${pgData.nextPage }'">다음</button>
		</c:if>
	</div>
	...
	<c:forEach var="board" items="${boardList}">
		...
		mib_category : ${board.getMib_category() }<br><br>
    </c:forEach>
    <br><br>
